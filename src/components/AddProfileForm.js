...
export default function AddProfile({ existingProfile = {} }) {
  ...
  const cleanBio = stripTags(bio).trim();

  const missingImage = !isEditMode && !img;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (missingImage) {
        throw new Error("Please choose an image under 1MB.");
      }
    ...
    <form onSubmit={handleSubmit} className={styles["add-profile"]}>
      ...
      {!isEditMode && !img && (
        <p className={styles.helperMessage}>
          Image is required. Please choose a JPG, PNG, or GIF under 1MB.
        </p>
      )}
      ...
      {isEditMode && img_url && (
        <div className={styles["current-image"]}>
          <p>Current Image:</p>
          <img src={img_url} alt="Current Profile" />
        </div>
      )}
